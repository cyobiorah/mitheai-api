...
export class SocialAccountService {
  ...
  async create(
    accountData: Omit<SocialAccount, "_id" | "createdAt" | "updatedAt">
    ...
    // Create social account - let MongoDB generate the ObjectId
  ...
  async update(
    ...
    if (!isValidObjectId(id)) {
      console.error(`Invalid ObjectId for social account update: ${id}`);
      return null;
    }
  ...
  async delete(id: string): Promise<boolean> {
    if (!isValidObjectId(id)) {
      console.error(`Invalid ObjectId for social account deletion: ${id}`);
      return false;
    }
  ...
  async refreshToken(
    ...
    if (!isValidObjectId(id)) {
      console.error(`Invalid ObjectId for token refresh: ${id}`);
      return null;
    }
  ...
  async transferOwnership(
    ...
    if (!isValidObjectId(id)) {
      console.error(`Invalid ObjectId for ownership transfer: ${id}`);
      return null;
    }
