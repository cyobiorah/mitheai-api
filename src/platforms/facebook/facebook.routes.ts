...
import passport from "../twitter/twitter.config";
  ...
  (req: any, res) => {
    ...
    try {
      if (!req.user) {
        ...
        return res.redirect(
          `${process.env.FRONTEND_URL}/account-setup?error=No%20user%20data`
        );
      }
      ...
      return res.redirect(
        `${process.env.FRONTEND_URL}/account-setup?error=Callback%20error`
      );
    }
  }
);
