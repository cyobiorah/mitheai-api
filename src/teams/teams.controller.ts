...
export const createTeam = async (req: Request, res: Response) => {
  try {
    ...
    const userId = req.user?._id; // Get the creator's ID from the request context

    // Validate required fields
    if (!name || !organizationId || !userId) {
      return res.status(400).json({
        error:
          "Missing required fields: name, organizationId, or user not authenticated",
      });
    }
    ...
    const newTeam = await teamService.create({
      ...
      description,
      organizationId,
      memberIds: [userId], // Add creator as first member
      settings: {
        permissions: ["basic"],
      },
    });
...
export const getTeams = async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      if (!teams) {
        console.log(
          `No teams found for organization ${organizationId}, returning empty array`
        );
        return res.json([]);
      }
      ...
      if (!Array.isArray(teams)) {
        console.warn(
          `Teams data for organization ${organizationId} is not an array, returning empty array`
        );
        return res.json([]);
      }
...
export const deleteTeam = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const members = await userService.findByTeam(teamId);
    ...
    for (const member of members) {
      ...
      if (isOrganizationUser(member)) {
        const updatedTeamIds =
          member.teamIds.filter(
            (tempTeamId: string) => tempTeamId !== teamId
          ) || [];
